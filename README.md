...
- Расширение файла не важно - это больше подсказка для пользователя, что файл хранит БД
- Файл БД содержит БД в *бинарном виде* - её нельзя редактировать через блокнот
...
> Здесь `SH()` - хэш-функция из библиотеки StringUtils, выполняемая на этапе компиляции. Переданная в неё строка не существует в программе - на этапе компиляции она превращается в число. Также можно использовать литерал `_h` - он делает же самое, как и макрос `H(имя)`.
...
int i = db["key1"];     // текст-ключ
float f = db[H(key2)];  // хэш-ключ
