...
### DBConnector
Автоматическое подключение к wifi по логину-паролю из базы данных. Если логин не задан - библиотека развернёт точку доступа для вашего веб-сервера (удобно использовать в паре с библиотекой [Settings](https://github.com/GyverLibs/Settings)). При ошибке подключения снова развернётся AP, при успешном подключении - нет. Подключение асинхронное с обработчиками. Опционально можно настроить сброс SSID при неудачном подключении, например при смене роутера устройство автоматически запустит AP для настройки нового.
```cpp
